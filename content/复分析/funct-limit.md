...
$\bold{x} = (x_1, x_2, \cdots, x_n)^T \in \mathbb{C}^n$
...
> proof:
> (1) $\longrightarrow$ (2):
> $\bold{x}^n \to \bold{y}$ means that $\forall \epsilon > 0, \exists N \in \mathbb{Z}_{\geq 0}$, s.t. $\forall n \geq N, \sqrt{\sum_{i=1}^{n} d(x_i^n,y_i)^2} < \epsilon$
...
Let $f: X \to \mathbb{C}^m, X \subseteq \mathbb{C}^n, f(\bold{x}) = (f_1(\bold{x}), f_2(\bold{x}), \cdots, f_m(\bold{x}))^T$
