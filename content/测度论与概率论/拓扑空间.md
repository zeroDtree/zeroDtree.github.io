...
4. 设$(X,\mathcal{U}),(Y, \mathcal{V})$为两个拓扑空间，$\mathcal{S},\mathcal{B}$分别为$Y$的子基和基，$f:X\to Y$,则以下个条件等价：
   - $f$连续
   - $\forall V \in \mathcal{V}, f^{-1}(V) \in \mathcal{U}$
   - $\forall S \in \mathcal{S}, f^{-1}(S)\in \mathcal{U}$
   - $\forall B \in \mathcal{B}, f^{-1}(B) \in \mathcal{U}$
5. $\bar{A}=A\cup A^d$
6. 拓扑空间中有序列收敛到$x$ $\Leftrightarrow$ x 为聚点。
7. $f:X \to Y$连续 $\Rightarrow$ 对于任意序列$\{x_n\}_{n=1}^\infty$，若$\{x_n\}_{n=1}^\infty$收敛到$x$，则$\{f(x_n)\}_{n=1}^\infty$收敛到$f(x)$
8. 若$X$为非空集合，$B\subseteq X$，如果$\mathcal{B}$是$B$的$X$上的某个拓扑的基，则 1)$\bigcup_{B\in \mathcal{B}}B=X$，2)$\forall B_1,B_2 \in \mathcal{B}$，$x\in B_1 \cap B_2 \Rightarrow \exists B_3 \in \mathcal{B}$，使得$x\in B_3 \subseteq B_1 \cap B_2$. 如果$\mathcal{E}$满足性质 1)和 2),则存在唯一拓扑$\tau=\{G\subseteq X \mid \exists \mathcal{E}_G \subseteq \mathcal{E}, G=\bigcup \mathcal{E}_G\}$以$\mathcal{E}$为基。
9. 若$X$为非空集合，$\mathcal{S}\subseteq \mathcal{P}(X) \wedge X = \bigcup_{S\in \mathcal{S}} S$，则存在唯一拓扑以 $\mathcal{S}$ 为子基。
10. 连续映射复合连续映射为连续映射
11. 坐标映射$\pi_t:Y=\prod_{t \in T} Y_t \to Y_t$为连续映射
12. $f:(X,\tau)\rightarrow (\prod_{t \in T} Y_t, \prod_{t \in T} \tau_t)$为连续映射 $\Leftrightarrow$ 对任意$t \in T$，$f_t = \pi_t \circ f:(X,\tau)\rightarrow (Y_t,\tau_t)$为连续映射
13. 对于双射$f:X \to Y$，以下三个命题等价：1) $f$为同胚；2) $f$为连续开映射；3) $f$为连续闭映射
14. 若$X$为第一可数空间，则$x\in X^d \Leftrightarrow A\backslash\{x\}$中有序列收敛于$x$
15. $N\in \mathcal{N}_x \Leftrightarrow \exists N_1 \in \mathcal{N}_x, N_1 \subseteq N$
16. 若$X$为第一可数空间，$f:X \to Y$为连续映射 $\Leftrightarrow$ 对于$X$中的序列$\{x_n\}_{n=1}^\infty$，若$\{x_n\}_{n=1}^\infty$收敛到$x$，则$\{f(x_n)\}_{n=1}^\infty$收敛到$f(x)$
17. 第二可数空间都是第一可数空间
18. 至多可数个第二可数空间的积空间仍为第二可数空间
19. 第二可数空间都是可分空间
20. 第一可数空间的子空间都为第一可数空间
21. 第二可数空间的子空间都为第二可数空间
22. 拓扑空间$X$为$T_1$空间 $\Leftrightarrow$ $X$的任意单点集为闭集。
23. $T_1$空间中，$A\subseteq X,x\in A^d \Leftrightarrow \forall U\in \mathcal{N}_x,U\cap A \text{是无限集}$
24. 拓扑空间为 Hausdorff 空间 $\Leftrightarrow$ 对角线$\Delta = \{(x,x) \mid x \in X\}$为闭集
25. Hausdorff 空间中极限唯一
26. $X,Y$为拓扑空间，且$Y$为 Hausdorff 空间，$f,g:X \to Y$为连续映射，则$A=\{x \in X \mid f(x)=g(x)\}$为闭集
27. $X,Y$为拓扑空间，且$Y$为 Hausdorff 空间，$f:D \to Y$为连续映射,$D$ 是$X$的稠密子集，则$f$唯一延拓到$X$上
28. $X$为紧空间 $\Leftrightarrow$ 具有有限交性质的闭集族的交不空
29. 设$X$为 Hausdorff 空间，$K$和$L$为$X$的紧集，且$K\cap L=\emptyset$，则存在开集$U,V$，使得$K\subseteq U,L\subseteq V,U\cap V=\emptyset$
30. 连续映射将紧集映射到紧集
31. Tychonoff 定理：任意紧空间的积空间为紧空间(未证明)
32. 第二可数空间都是 Lindelöf 空间
33. $X$为拓扑空间，$A\subseteq X$，$A$为可数紧的 $\Leftrightarrow$ 相对拓扑$A$的任意开覆盖都有可数子覆盖
34. $X$为可数紧空间，$A$是$X$的无限子集，则$A^d\not= \emptyset$
35. $K$紧，$U$开，则$K\backslash U$紧
36. 紧集的闭子集为紧集
37. 可数紧集的闭子集为可数紧集
38. 紧集都是列紧的
39. 紧 Hausdorff 空间为 正规空间
40. Hausdorff 空间中，紧集都是闭集
41. 设$X$为 Hausdorff 空间，$K$为$X$的紧集，$U_1,U_2$均为$X$的开集，且$K\subseteq U_1 \cup U_2$，则存在紧集$K_1,K_2$，使得$K_1 \subseteq U_1,K_2 \subseteq U_2,K=K_1 \cup K_2$
42. 第一可数空间中，紧 $\Rightarrow$ 列紧
43. 度量空间中，任何列紧闭集都存在 Lebesgue 数
44. 度量空间中$X$，$A\subseteq X$，$A$完全有界 $\Leftrightarrow$ $A$包含基本子序列(Cauchy sequence)
45. 度量空间中，紧 $\Leftrightarrow$ 列紧闭
46. 第二可数空间的任何基都存在一个子集为可数基
47. $X$不连通 $\Leftrightarrow$ $X$存在既开又闭的非空真子集
48. 若$C,D$是$X$的分割，$Y$是$X$的连通子集，则$Y\subseteq C \vee Y\subseteq D$
49. 设$\mathcal{A}=\{A_\alpha|\alpha \in J\}$为$X$的连通子集族，且$\bigcap_{\alpha \in J} A_\alpha \not= \emptyset$，则$\bigcup_{\alpha \in J} A_\alpha$为连通子集
50. $A,B\subseteq X$,$A\subseteq B$,则$A$在$B$中连通等价于$A$在$X$中连通
51. 连续映射将连通集映射为连通集
52. $(X_0,X_0 \cap \tau)$中的(诱导、相对)开集是$X_0$的开集 $\Leftrightarrow$ $X_0$是$X$的开集
53. Hausdorff 空间中,以下三个命题等价
    (a) $X$为局部紧空间
    (b)$X$中任意一点都存在预紧开邻域
    (c)$X$上存在一个预紧基(由预紧开集构成的基)
54. $X$ 为 Hausdorff 空间，若$X$在点$x$处局部紧，则对于$x$的任意开邻域$U_x$,都存在$x$的开邻域$V_x$，使得$\overline{V}_x \subseteq U_x$且$V_x$为紧集
55. 若$X$为 局部紧 Hausdorff 空间，$A$为$X$中的开集或闭集，则子空间$A$为局部紧 Hausdorff 空间
