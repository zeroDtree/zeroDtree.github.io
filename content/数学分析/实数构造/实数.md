...
**Lemma** (Existence of $n$th roots). *Let $x \ge 0$ be a non-negative real, and let $n \ge 1$ be a positive integer. Then the set $E := \{y \in \mathbf{R} : y \ge 0 \text{ and } y^n \le x\}$ is nonempty and is also bounded above. In particular, $x^{1/n}$ is a real number.*
...
**Lemma** *Let $x, y \ge 0$ be non-negative reals, and let $n, m \ge 1$ be positive integers.*
