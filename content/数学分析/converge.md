...
**极限**：称$L$为数列$a_n$的极限，若$a_n$收敛于$L$,记作$\lim\limits_{n \to \infty} a_n = L$
...
**上极限**：$L^+:= \mathop{\lim \sup}\limits_{n \to \infty} a_n := \inf\limits_{n\geq m} \sup\limits_{k\geq n} a_k$

**下极限**：$L^-:= \mathop{\lim \inf}\limits_{n \to \infty} a_n := \sup\limits_{n\geq m} \mathop{\inf}\limits_{k\geq n} a_k$
...
- $\lim\limits_{n \to \infty} \max(a_n,b_n) = \max(\lim\limits_{n \to \infty} a_n, \lim\limits_{n \to \infty} b_n)$
...
- $a_n \leq b_n \implies \sup(a_n) \leq \sup(b_n) \wedge \inf(a_n) \leq \inf(b_n) \wedge \limsup(a_n) \leq \limsup(b_n) \wedge \liminf(a_n) \leq \liminf(b_n)$
- 若$a_n \leq b_n \leq c_n$,且$a_n,c_n$收敛于$x$,则$b_n$收敛于$x$
- 若$x \in \mathbb{R}$，则$a_n$收敛于$x \iff \forall \epsilon > 0, \exists N \in \mathbb{Z}_{\geq m}, \forall n \geq N, |a_n - x| < \epsilon$
- $a_n \to 0$ $\iff$ $|a_n| \to 0$
- 实数域的完备性：若$a_n \in \mathbb{R}$, 则$a_n$是 cauchy 列 $\iff$ $a_n$收敛于$\mathbb{R}$中的某个数
- $\inf\limits_{n\geq m} \sup\limits_{k\geq n} a_k = \inf\limits_{n\geq m_0} \sup\limits_{k\geq n} a_k$
- $\sup\limits_{n\geq m} \inf\limits_{k\geq n} a_k = \sup\limits_{n\geq m_0} \inf\limits_{k\geq n} a_k$
